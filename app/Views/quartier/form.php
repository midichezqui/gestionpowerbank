...
$actionUrl = $isEdit
    ? "index.php?controller=quartier&action=edit&id=" . $quartier->id
    : "index.php?controller=quartier&action=create";

function q_field($name, $old, $quartier)
{
    if (isset($old[$name])) return $old[$name];
    if ($quartier) return $quartier->$name;
    return '';
}

$selectedCommune = isset($old['idCommune'])
    ? $old['idCommune']
    : ($quartier ? $quartier->idCommune : '');
?>
...
<div class="p-4 sm:p-6 max-w-3xl mx-auto">
    ...
    <h1 class="text-xl sm:text-2xl font-semibold text-slate-900 mb-4 flex items-center gap-2">
        ...
        <span><?= $isEdit ? 'Modifier un quartier' : 'Ajouter un quartier' ?></span>
    </h1>
        ...
        <form action="<?= $actionUrl ?>" method="post" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="grid gap-1">
                    ...
                    <input type="text"
                           name="nomQuartier"
                           value="<?= htmlspecialchars(q_field('nomQuartier', $old, $quartier)) ?>"
                    ...
                    <select name="idCommune"
                        ...
                        <?php foreach ($communes as $c): ?>
                            ...
                            <option value="<?= htmlspecialchars($value) ?>"
                                <?= (string)$value === (string)$selectedCommune ? 'selected' : '' ?>>
