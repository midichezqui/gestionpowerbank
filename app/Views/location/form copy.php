...
<div class="p-4 sm:p-6">
    ...
    <div class="max-w-3xl mx-auto bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="px-4 sm:px-6 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <div>
                <h1 class="text-lg sm:text-xl font-bold text-slate-900 flex items-center gap-2">
                    <i class="fa-solid fa-file-circle-plus text-sky-600"></i>
            ...
            <a href="index.php?controller=location&action=index"
               class="inline-flex items-center gap-2 text-xs sm:text-sm px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50">
        ...
        <div class="px-4 sm:px-6 py-5">
            ...
            <form action="" method="post" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="grid gap-1">
                            <label class="text-xs font-medium text-slate-700">
                                Client (Quartier) <span class="text-red-500">*</span>
                            </label>
                            <p class="text-[11px] text-slate-400 mb-1">
                                Seuls les clients dont les quartiers sont concernés par vos affectations d'aujourd'hui
                                sont affichés.
                            </p>
                            <select name="idClient"
                                ...
                                foreach ($clients as $cl):
                                    $id      = (string)$cl['id'];
                                    $nomComplet = trim($cl['prenom'] . ' ' . $cl['nom'] . ' ' . $cl['postnom']);
                        ...
                        <div class="grid gap-1">
                            <label class="text-xs font-medium text-slate-700">
                                Affectation (Agent - Quartier - PowerBank) <span class="text-red-500">*</span>
                            </label>
                            <p class="text-[11px] text-slate-400 mb-1">
                                Seules vos affectations pour la date <?= htmlspecialchars($dateLocation) ?> sont affichées.
                            </p>
                            <select name="idAffectation"
                                ...
                                foreach ($affectations as $af):
                                    $id   = (string)$af['id'];
                                    $txt  = $af['agentNom'] . ' | ' . $af['quartierNom'] . ' | ' . $af['powerCode'];
                                ?>
                                    <option value="<?= htmlspecialchars($id) ?>"
                ...
                <!-- Boutons -->
                <div class="pt-4 border-t border-slate-100 mt-2 flex flex-col sm:flex-row gap-2 sm:gap-3">
                    <button type="submit"
