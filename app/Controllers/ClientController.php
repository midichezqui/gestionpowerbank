...
class ClientController extends Controller
{
    ...
    public function delete()
    {
        ...
        // Seul le super (1) peut supprimer un client
    ...
    public function debloquer()
    {
        ...
        // On récupère la dernière location avec pénalité de ce client
        $sql = "
            SELECT id, penalite
            FROM location
        ...
        if (!$loc) {
            // Aucun enregistrement avec pénalité
            echo "Aucune pénalité trouvée pour ce client.";
            return;
        }

        $penalite       = (int)$loc['penalite'];          // ex 2000
        $montantADonner = $penalite * 2;                  // ex 4000

        // 👉 Ici tu peux afficher une vue avec le montant à payer
        // et un bouton "Confirmer paiement effectué"
        // Pour simplifier, supposons que tu confirmes direct :
        
        // 1. Débloquer le client
        $stmtUpd = $pdo->prepare("UPDATE clients SET idEtat = 1 WHERE id = :idClient");
        $stmtUpd->execute(['idClient' => $idClient]);

        // 2. (Optionnel) marquer la pénalité comme payée sur la location
        $stmtLoc = $pdo->prepare("
            UPDATE location
            SET penalite = 0
            WHERE id = :idLocation
        ");
        $stmtLoc->execute(['idLocation' => $loc['id']]);

        // 3. Redirection
        header('Location: index.php?controller=client&action=show&id=' . $idClient);
        exit;
    }
